...
int main(){
    vector<Map> maps;
    vector<long long> seeds, locations, numbers;
    string line; 
    ...
    // Extract seed and range values
    for (size_t i = 0; i < numbers.size(); i += 2) {
        int seed = numbers[i];
        int range = numbers[i + 1];

        // Generate and store the range of numbers in the seeds vector
        for (int j = 0; j < range; ++j) {
            int generatedNumber = seed + j;
            seeds.push_back(generatedNumber);
        }
    }
    while(!infile.eof()){
    ...
    cout<<findLowestValue(locations);
